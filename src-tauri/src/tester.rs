...
use crate::{
    client::OpenAIClient,
    constants::{DOG_INFO_COLLECTION, MAX_REQUEST_DEFENCE},
    models::{OddsRange, RequestsInfo, Settings, TestDateTime, TestResults},
...
impl Tester {
    ...
    async fn generate_races(&self) -> Result<RequestsInfo> {
        ...
        for (idx, race_id) in race_ids.into_iter().enumerate() {
            ...
            let race_docs: Vec<Document> = collection.find(filter).await?.try_collect().await?;
            ...
            for dog_result in &race_docs {
                ...
                form_docs.sort_by(|a, b| {
                    b.get_datetime("raceDateTime")
                        .unwrap()
                        .cmp(a.get_datetime("raceDateTime").unwrap())
                });
